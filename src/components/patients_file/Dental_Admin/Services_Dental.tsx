...
const Services_Dental: React.FC = () => {
  ...
  // CLEANUP DUPLICATES (ONE-TIME)
  const cleanupDuplicates = async () => {
    if (!window.confirm("This will permanently delete duplicate dental services (keeps the first one). Continue?")) return;

    const q = query(collection(db, "DentalServices"), where("department", "==", "Dental"));
    ...
    toDelete.forEach((id) => batch.delete(doc(db, "DentalServices", id)));
    await batch.commit();
  ...
  // SEED DEFAULTS + REAL-TIME LISTENER
  useEffect(() => {
    const seedDefaultServices = async () => {
      const colRef = collection(db, "DentalServices");
      const q = query(colRef, where("department", "==", "Dental"));
      const snapshot = await getDocs(q);
      ...
      Object.entries(DEFAULT_DENTAL_SERVICES).forEach(([category, names]) => {
      ...
      if (added > 0) {
        await batch.commit();
        console.log(`Seeded ${added} missing dental services`);
      }
    };
  ...
  const handleSaveService = async () => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "DentalServices", editingId), { name, category: cat });
      } else {
        await addDoc(collection(db, "DentalServices"), {
          name,
          category: cat,
          enabled: true,
          department: "Dental",
  ...
  return (
    <div className="dashboards">
      ...
      <aside className="sidebars">
        <div>
          <div className="logo-boxs" onClick={() => navigate("/dashboard_dental")} style={{ cursor: "pointer" }}>
          ...
          <nav className="nav-linkss">
            {[
              { to: "/dashboard_dental", icon: FaTachometerAlt, label: "Dashboard" },
              { to: "/appointments_dental", icon: FaCalendarAlt, label: "Appointments" },
              { to: "/patientrecords_dental", icon: FaUsers, label: "Patient Records" },
              { to: "/manageslots_dental", icon: FaClock, label: "Manage Slots" },
              { to: "", icon: FaTooth, label: "Services", active: true },
              { to: "/reports&analytics_dental", icon: FaChartBar, label: "Reports & Analytics" },
            ].map((item) => (
              <div
                key={item.to}
      ...
      <main className="main-content">
        <div className="top-navbar-clinical">
          <h5 className="navbar-title">Dental Services Management</h5>
        ...
        <div className="content-wrapper" style={{ padding: "20px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "20px" }}>
            <h3>{showTrash ? "Trashed Services" : "All Dental Services"}</h3>
            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
              ...
              {!showTrash && (
                <button
                  ...
                  style={{ background: "#005b9f", color: "white", border: "none", padding: "10px 16px", borderRadius: "8px", display: "flex", alignItems: "center", gap: "8px" }}
                >
                  <FaPlus /> Add Service
                </button>
              )}
      ...
      {showAddModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }} onClick={closeModal}>
          <div style={{ background: "white", padding: "24px", borderRadius: "12px", width: "90%", maxWidth: "500px" }} onClick={(e) => e.stopPropagation()}>
            <h3>{editingId ? "Edit Service" : "Add New Dental Service"}</h3>
            ...
            <div style={{ display: "flex", gap: "12px", justifyContent: "flex-end" }}>
              <button onClick={closeModal} style={{ padding: "10px 16px", border: "1px solid #ccc", borderRadius: "8px", background: "white" }}>
                Cancel
              </button>
...
export default Services_Dental;