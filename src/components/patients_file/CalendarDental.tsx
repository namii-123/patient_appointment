...
const CalendarDental: React.FC<CalendarDentalProps> = ({ formData, onNavigate }) => {
  ...
  const handleSelectDate = async (day: number) => {
    ...
    if (snap.exists()) {
      ...
      // First time → create default limited slots (Dental = 3 per slot)
      slots = predefinedTimes.map((time) => ({
        slotID: `SLOT-${uid.randomUUID()}`,
        time,
        remaining: 3,
  ...
  return (
    <div className="calendar-container pb-24">
      <h2>Select Appointment Date (Dental)</h2>
          ...
          <button
            ...
            onClick={() => {
              const msg = formData?.fromReview
                ? `Update Dental appointment?\n\n${selectedDate} | ${selectedSlot?.time}\n\nReturn to review?`
                : `Proceed with Dental slot?\n\n${selectedDate} | ${selectedSlot?.time}`;
