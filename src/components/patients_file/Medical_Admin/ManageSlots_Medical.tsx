...
const ManageSlots_Medical: React.FC = () => {
  ...
  const [notifications, setNotifications] = useState<Notification[]>([
    ...
    { text: "Radiology update completed", unread: false },
  ]);
  ...
  // Load slots — default to UNLIMITED if no data exists
  useEffect(() => {
  ...
  const handleDayClick = (date: Date) => {
    ...
    if (!dayData.unlimited && !dayData.closed && dayData.slots?.length > 0) {
      dayData.slots.forEach(s => {
        counts[s.time] = s.remaining;
      });
    } else if (!dayData.unlimited && !dayData.closed) {
  ...
  const handleSave = async () => {
    ...
    if (closed) {
      payload = { slots: [], totalSlots: 0, unlimited: false, closed: true };
    } else if (unlimited) {
      payload = { slots: [], totalSlots: 999, unlimited: true, closed: false };
    } else {
      ...
      payload = { slots: slotsData, totalSlots: total, unlimited: false, closed: false };
    }
    ...
    try {
      ...
      openCustomModal("Failed to save slots. Please try again.", "error");
    }
  };
      ...
      <aside className="sidebars">
        <div>
          ...
          <nav className="nav-linkss">
            <div className="nav-item"><FaTachometerAlt className="nav-icon" /><span onClick={() => navigate("/dashboard_medical")}>Dashboard</span></div>
            <div className="nav-item"><FaCalendarAlt className="nav-icon" /><span onClick={() => navigate("/appointments_medical")}>Appointments</span></div>
            <div className="nav-item"><FaUsers className="nav-icon" /><span onClick={() => navigate("/patientrecords_medical")}>Patient Records</span></div>
            <div className="nav-item active"><FaClock className="nav-icon" /><span>Manage Slots</span></div>
            <div className="nav-item"><FaStethoscope className="nav-icon" /><span onClick={() => navigate("/services_medical")}>Services</span></div>
            <div className="nav-item"><FaChartBar className="nav-icon" /><span onClick={() => navigate("/reports&analytics_medical")}>Reports & Analytics</span></div>
          </nav>
        </div>
        <div className="sidebar-bottom">
          ...
          <div className="signout-box">
            ...
            <span
              onClick={() => openCustomModal("Are you sure you want to sign out?", "confirm", async () => {
                try {
                  await signOut(auth);
                  navigate("/loginadmin", { replace: true });
                } catch (error) {
                  openCustomModal("Sign out failed.", "error");
                }
              })}
              ...
              {days.map(day => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="slots-summary">{displayText}</div>
                  </div>
                );
              })}
            </div>
                ...
                <div className="radiology-modal-content" onClick={e => e.stopPropagation()}>
                  ...
                  <div className="radiology-modal-footer">
                    {customModalType === "confirm" ? (
                      <>
                        <button className="radiology-modal-btn cancel" onClick={closeCustomModal}>No, Cancel</button>
...
export default ManageSlots_Medical;