...
const ReportsAnalytics_DDE: React.FC = () => {
  ...
  const filteredData = useMemo(() => {
    ...
    if (year) {
      filtered = filtered.filter((item) => item.requestDate.includes(year));
    }
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      let tableHTML = `
        <thead>
          <tr style="background-color: #f2f2f2;">
            <th style="border: 1px solid #ddd; padding: 8px; text-align: center;">Request Date</th>
      ...
      filteredData.forEach((row, idx) => {
        ...
        tableHTML += `
          <tr>
            <td style="border: 1px solid #ddd; padding: 8px; text-align: center;">${row.requestDate}</td>
