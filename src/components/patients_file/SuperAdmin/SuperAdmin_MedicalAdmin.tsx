...
interface Admin {
  ...
  email: string;
  role: string;
  status: "Approved" | "Rejected" | "Pending" | "Not Active";
  contact?: string;
  department?: string;
  date?: string;
  username?: string;
...
const SuperAdmin_MedicalAdmin: React.FC = () => {
  ...
  const notifications = [
    { id: 1, text: "New admin registered in Medical", unread: true },
    { id: 2, text: "2 Admin requests pending approval", unread: true },
    { id: 3, text: "Report submitted by Medical Admin", unread: false },
  ...
  useEffect(() => {
    ...
    const userAdminQuery = query(collection(db, "UserAdmin"), where("department", "==", "Medical"));

    const updateAdmins = (newAdmins: Admin[]) => {
      setAdmins((prev) => {
        const allAdmins = [...prev, ...newAdmins];
        const uniqueAdmins = Array.from(new Set(allAdmins.map(a => a.id))).map(id => allAdmins.find(a => a.id === id)!);
        return uniqueAdmins;
      });
      setLoading(false);
    };
  ...
  d.status?.toLowerCase() === "approved"
    ...
    ? "Rejected"
    : d.status?.toLowerCase() === "not active"
    ? "Not Active"
    : "Pending",
          contact: d.contact || "",
    ...
    const unsubscribeUser = onSnapshot(userAdminQuery, (snapshot) => {
      const data = snapshot.docs.map((doc) => {
        const d = doc.data();
        const createdAtValue = d.createdAt ? (d.createdAt.toDate ? d.createdAt.toDate() : new Date(d.createdAt)) : null;
        return {
            ...
            {showNotifications && (
              <div className="notification-dropdown">
                <div className="notification-header">
                  ...
                  <button className="mark-read-btn" onClick={() => notifications.forEach(n => n.unread = false)}>Mark all as read</button>
                </div>
                {notifications.length > 0 ? notifications.map(n => <div key={n.id} className={`notification-item ${n.unread ? "unread" : ""}`}><span>{n.text}</span>{n.unread && <span className="notification-badge">New</span>}</div>) : <div className="notification-empty">No notifications</div>}
              </div>
            )}
          </div>
        </div>

        <button className="back-btn" onClick={() => handleNavigation("/superadmin_dashboard")}><FaArrowLeft /> Back</button>

        <div className="filters-container-clinical">
          <div className="filter-clinical"><label>Date:</label>
            <select id="month" value={selectedMonth} onChange={e => setSelectedMonth(e.target.value)}>
              <option value="">Month</option>
              {Array.from({ length: 12 }, (_, i) => <option key={i+1} value={(i+1).toString().padStart(2, "0")}>{new Date(0,i).toLocaleString('default', { month: 'long' })}</option>)}
            </select>
          </div>
            ...
            <tbody>
              {filteredAdmins.length > 0 ? filteredAdmins.map(a => (
                <tr key={a.id}>
                  ...
                  <td>{a.email}</td>
                  <td><span className={`status-badge ${a.status.toLowerCase()}`}>{a.status.charAt(0).toUpperCase()+a.status.slice(1)}</span></td>
