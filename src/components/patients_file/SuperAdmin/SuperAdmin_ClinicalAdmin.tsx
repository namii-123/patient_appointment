...
const SuperAdmin_ClinicalAdmin: React.FC = () => {
  ...
  const notifications = [
    { id: 1, text: "New admin registered in Clinical", unread: true },
    { id: 2, text: "2 Admin requests pending approval", unread: true },
    { id: 3, text: "Report submitted by Clinical Admin", unread: false },
 ...
 const [yearOptions, setYearOptions] = useState<number[]>(() => {
  ...
  return Array.from({ length: 11 }, (_, i) => current - 5 + i); // 5 years back + 5 forward
});
...
const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
  ...
  useEffect(() => {
    setLoading(true);
    const manageAdminsQuery = query(collection(db, "ManageAdmins"), where("department", "==", "Clinical Laboratory"));
    const userAdminQuery = query(collection(db, "UserAdmin"), where("department", "==", "Clinical"));
    ...
    <div className="dashboard">
      ...
      <main className="main-content-superadmin">
        ...
        <div className="top-navbar-superadmin">
          <h5 className="navbar-title">Clinical Admins</h5>
    ...
    {filteredAdmins.length > 0 ? (
      ...
      <tr>
        <td colSpan={8} style={{ textAlign: "center", padding: "12px" }}>
          {loading ? "Loading..." : "No Clinical admins found."}
        </td>
      </tr>
    )}
  </tbody>
...
export default SuperAdmin_ClinicalAdmin; 
