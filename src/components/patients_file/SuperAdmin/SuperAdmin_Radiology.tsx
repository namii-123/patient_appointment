...
import "../assets/SuperAdmin_Radiology.css";
...
const SuperAdmin_Radiology: React.FC = () => {
  ...
  // Count totals
  const pendingCount = radiologyAppointments.filter((a) => a.status === "pending").length;
  const approvedCount = radiologyAppointments.filter((a) => a.status === "approved").length;
  const completedCount = radiologyAppointments.filter((a) => a.status === "completed").length;
  const rejectedCount = radiologyAppointments.filter((a) => a.status === "rejected").length;

  // Apply filter
  const filteredAppointments =
    filter === "all"
      ? radiologyAppointments
      : radiologyAppointments.filter((a) => a.status === filter);
      ...
      <main className="main-content-superadmin">
        ...
        <button className="back-btn" onClick={() => handleNavigation("/superadmin_dashboard")}>
          <FaArrowLeft /> Back
                        ...
                        {/* Date Filter */}
<div className="filters-container-radiology">
  <div className="filter-radiology">
    <label>Date:</label>
    <select
      id="month"
      value={selectedMonth}
    ...
    >
      ...
      {Array.from({ length: 6 }, (_, i) => {
        ...
        <div className="summary-cards">
          ...
          >
            <h3>{radiologyAppointments.length}</h3>
            <p>All</p>
          </div>
        </div>
