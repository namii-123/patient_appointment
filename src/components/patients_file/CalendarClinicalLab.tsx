...
const CalendarClinicalLab: React.FC<AppointmentCalendarProps> = ({
  formData,
  onConfirm,
  onNavigate,
}) => {
...
useEffect(() => {
      ...
      for (let d = 1; d <= totalDays; d++) {
        ...
        const unsub = onSnapshot(
          slotRef,
          (slotDoc) => {
            console.log(`CalendarClinical: Date ${dateKey}, Firestore data:`, slotDoc.data());
            setSlots((prev) => ({
  ...prev,
  [d]: slotDoc.exists()
    ? slotDoc.data().closed
...
setIsClosed((prev) => ({
  ...
  return Object.entries(
    slots.reduce((acc, slot) => {
      if (!acc[slot.time]) {
        acc[slot.time] = { time: slot.time, remaining: 0 };
      }
      acc[slot.time].remaining += slot.remaining;
      return acc;
    }, {} as Record<string, { time: string; remaining: number }>)
  ).map(([time, data]) => ({
    slotID: `AGG-${time.replace(/\s+/g, "-")}`, // consistent ID per time
    time,
...
const handleSelectDate = async (day: number) => {
  ...
  if (slotDoc.exists() && slotDoc.data().closed) {
    console.log(`CalendarClinical: Date ${selected} is closed`);
    return;
  }
  setSelectedDate(selected);
  let slotsData: Slot[];
  if (slotDoc.exists() && !slotDoc.data().closed) {
  let slotsData = slotDoc.data().slots as Slot[];

  // 🔹 Ensure every slot has a slotID
  slotsData = slotsData.map((s) =>
    s.slotID ? s : { ...s, slotID: `SLOT-${uid.randomUUID()}` }
  );

  console.log(`CalendarClinical: Loaded slots for ${selected}:`, slotsData);
  const aggregatedSlots = aggregateSlotsByTime(slotsData);
  if (aggregatedSlots.every((s) => s.remaining === 0)) {
    console.log(`CalendarClinical: No slots available for ${selected}`);
    return;
  }
    ...
    console.log(`CalendarClinical: Generated slots for ${selected}:`, slotsData);
  ...
  if (!slotDoc.exists() || slotDoc.data().closed) {
    console.log(`CalendarClinical: Slot document for ${selectedDate} does not exist or is closed`);
    setShowModal(false);
    return;
  }

  let currentSlots = slotDoc.data().slots as Slot[];
  ...
  if (!availableSlot) {
    console.log(`CalendarClinical: No available slot for ${slotTime}`);
    setShowModal(false);
    return;
  }
  ...
  if (formData?.appointmentId) {
    transaction.update(doc(db, "Appointments", formData.appointmentId), {
      ...
      updatedAt: new Date().toISOString(),
        ...
        console.log(`CalendarClinical: Slot booked for ${selectedDate}, slotID: ${selectedSlot?.slotID}`);
        if (onConfirm && selectedSlot) {
          onConfirm(selectedDate, selectedSlot.slotID);
        }
      } catch (error) {
        console.error("CalendarClinical: Error updating slot:", error);
        setShowModal(false);
