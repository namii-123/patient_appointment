  ...
  const handleSelectSlot = async (slotTime: string) => {
    ...
    try {
      await runTransaction(db, async (transaction) => {
        ...
        if (!slotDoc.exists() || slotDoc.data().closed) {
          console.log(`CalendarClinicalLab: Slot document for ${selectedDate} does not exist or is closed`);
          setShowModal(false);
          return;
        }
        ...
        if (!availableSlot) {
          console.log(`CalendarClinicalLab: No available slot for ${slotTime}`);
          setShowModal(false);
          return;
        }
      ...
      console.log(`CalendarClinicalLab: Slot booked for ${selectedDate}, slotID: ${selectedSlot?.slotID}`);
    } catch (error) {
      console.error("CalendarClinicalLab: Error updating slot:", error);
      setShowModal(false);
