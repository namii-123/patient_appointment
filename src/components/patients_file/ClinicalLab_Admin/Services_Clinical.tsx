...
const Services_Clinical: React.FC = () => {
   ...
   useEffect(() => {
     ...
     playNotificationSound();  
       ...
       setAdminNotifications(prev => {
         ...
         newNotifications.forEach(n => map.set(n.id!, n));
         return Array.from(map.values()).sort((a, b) =>
           (b.timestamp?.toDate?.() || b.timestamp || 0) - 
           (a.timestamp?.toDate?.() || a.timestamp || 0)
         );
       });
  ...
  useEffect(() => {
    const seedDefaultServices = async () => {
      const colRef = collection(db, "ClinicalServices");
      const q = query(colRef, where("department", "==", "Clinical Laboratory"));
      const snapshot = await getDocs(q);
      ...
      Object.entries(DEFAULT_CLINICAL_SERVICES).forEach(([category, names]) => {
      ...
      if (added > 0) {
        await batch.commit();
        console.log(`Seeded ${added} missing clinical services`);
      }
    };
  ...
  // CRUD Functions (same as before, just cleaned)
  const handleSaveService = async () => {
    ...
    try {
      if (editingId) {
        await updateDoc(doc(db, "ClinicalServices", editingId), { name, category: cat });
      } else {
        await addDoc(collection(db, "ClinicalServices"), {
          name,
          category: cat,
          enabled: true,
          department: "Clinical Laboratory",
  ...
  const toggleService = async (id: string, cur: boolean) => {
    await updateDoc(doc(db, "ClinicalServices", id), { enabled: !cur });
  };
      ...
      <aside className="sidebars">
        <div>
          ...
          <nav className="nav-linkss">
            {[
              { to: "/dashboard_clinical", icon: FaTachometerAlt, label: "Dashboard" },
              { to: "/appointments_clinical", icon: FaCalendarAlt, label: "Appointments" },
              { to: "/patientrecords_clinical", icon: FaUsers, label: "Patient Records" },
              { to: "/manageslots_clinical", icon: FaClock, label: "Manage Slots" },
              { to: "", icon: FaStethoscope, label: "Services", active: true },
              { to: "/reports&analytics_clinical", icon: FaChartBar, label: "Reports & Analytics" },
            ].map((item) => (
              <div
                key={item.to}
    ...
    async () => {
      ...
      <main className="main-content">
        ...
        <div className="top-navbar-clinical">
          <h5 className="navbar-title">Clinical Services Management</h5>
...
export default Services_Clinical;