...
useEffect(() => {
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      const message =
        newStatus === "Approved"
          ? `We are pleased to inform you that your appointment scheduled for ${appointment.date} at ${appointment.slotTime} has been approved. Please arrive on time and bring the downloaded outpatient request form.\n\nThank you for choosing our services.\n\nBest regards,\nClinical Team`
  ...
  try {
    ...
    if (slotSnap.empty) {
      ...
      timesToBook.forEach(time => {
        const slotRef = doc(collection(db, "Slots_Clinical"));
        batch.set(slotRef, {
          date: rescheduleDate,
          time: time,
          isBooked: true,
    ...
    const message = `Your appointment has been RESCHEDULED by the Clinical Department.\n\nNew Schedule:\nDate: ${rescheduleDate}\nTime: ${start12} - ${endTimeDisplay} (2-hour block)\n\nPlease arrive on time.\n\nThank you!\nClinical Team`;
