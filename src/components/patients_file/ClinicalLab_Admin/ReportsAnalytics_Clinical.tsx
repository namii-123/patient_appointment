...
const templatePage = copiedPages[0]; 
      ...
      drawCentered("CLINICAL LABORATORY REPORTS", y, 18, true);
      y -= 30;
...
const safeBuffer = uint8.buffer.slice(0); 
  ...
  const filename = `Clinical_Report_${year || "All"}_${month || "All"}_${status}_${new Date().toISOString().slice(0,10)}.pdf`;
...
const handleDownloadPDFReport = async () => {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      drawCenteredText("CLINICAL LABORATORY REPORTS", y, 18, true);
      y -= 30;
      ...
      const filename = `Clinical_Report_${year || "AllTime"}_${month || "All"}_${status}_${new Date().toISOString().slice(0,10)}.pdf`;
...
// Helper para sa page numbers (same sa Appointments)
