...
// SUMMARY BASED ON CURRENT FILTERS (Year, Month, Status)
const filteredSummary = useMemo(() => {
  ...
  filteredData.forEach((day) => {
    if (status !== "all") {
      // Kung specific status ang gipili (e.g. Completed lang)
      const value = (day as any)[status] || 0;
      total += value;

      // Individual counts based sa gipili nga status
      ...
      // Kung "All" — i-total tanan except rejected
...
const templatePage = copiedPages[0]; 
      ...
      drawCentered("RADIOLOGY REPORTS", y, 18, true);
      y -= 30;
      ...
      // Convert pdfBytes to a safe ArrayBuffer
const uint8 = new Uint8Array(pdfBytes);
const safeBuffer = uint8.buffer.slice(0); // ← FIXES THE ERROR
  ...
  const filename = `Radiology_Report_${year || "All"}_${month || "All"}_${status}_${new Date().toISOString().slice(0,10)}.pdf`;
...
const handlePrint = () => {
    ...
    input.onchange = (e) => {
      ...
      if (file) injectDataIntoTemplate(file, true); // true = print mode
    };
...
const handleDownloadPDFReport = async () => {
  ...
  input.onchange = async () => {
    ...
    try {
      ...
      drawCenteredText("RADIOLOGY REPORTS", y, 18, true);
      y -= 30;
      ...
      // === DRAW SUMMARY BOX (MOVE TO NEXT PAGE IF NEEDED) ===
      ...
      const filename = `Radiology_Report_${year || "AllTime"}_${month || "All"}_${status}_${new Date().toISOString().slice(0,10)}.pdf`;
